...
public class outer extends JFrame {
    ...
    public outer(){
        super("приложение");
        super.setBounds(200, 100, 1100, 500);
        super.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

        container = super.getContentPane();
        container.setLayout(new GridLayout(2, 6, 0, 20));
        init();
    ...
    private void guess() {
        ...
        try {
            inner.setMinNumb(Integer.parseInt(minNumb.getText()));
            inner.setMaxNumb(Integer.parseInt(maxNumb.getText()));
            System.out.println();
        } catch (Exception exc) {
            area.append(exc.getMessage());
            area.append(System.lineSeparator());
        ...
        estimatedNumb = inner.guess();
        area.setText("");
        area.append("Возможно, что это ... ");
    ...
    private void guessLess() {
        if (inner.getMinNumb() == inner.getMaxNumb()) {
            youAreCheater();
        } else {
